...
app.get('/team', async(req,res) => {
    try{
        const allItem = await teamModel.find({})
...
app.get('/readOne/:id', async(req,res) =>{
    try{
        ...
        const itemData = await teamModel.findById({_id:itemId})
...
app.delete('/delete/:id', async(req,res) =>{
    try{
        const itemId = req.params.id
        await teamModel.deleteOne({_id:itemId})
...
app.patch("/update/:id", upload.single("photo"), async (req, res) => {
    try {
        ...
        const itemUpdated =  await teamModel.findByIdAndUpdate({ _id: itemId }, {$set:itemInfo}, {new:true})
